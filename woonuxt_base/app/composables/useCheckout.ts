...
const buildCheckoutPayload = (isPaid = false): CheckoutInput => {
  ...
  // ✅ CORRECTION CRITIQUE : Prioriser orderInput.value (nos données fraîches du formulaire)
  // et utiliser customer.value uniquement comme fallback de sécurité.
  const billing = orderInput.value.billing || customer.value?.billing || {};
  const shipping = shipToDifferentAddress 
    ? (orderInput.value.shipping || customer.value?.shipping || billing) 
    : billing;
  ...
  async function openPayPalWindow(redirectUrl: string): Promise<boolean> {
   ...
   const processCheckout = async (isPaid = false): Promise<any> => {
    ...
    try {
      ...
      // ✅ 1. Cas où l'objet 'order' est manquant mais 'redirect' (WordPress) est présent
      if (!orderId || !orderKey) {
        if (checkout?.redirect && import.meta.client) {
          try {
            const wpUrl = new URL(checkout.redirect);
            
            // Extraire l'orderId du chemin
            const pathParts = wpUrl.pathname.split('/').filter(Boolean);
            const extractedOrderId: string = pathParts[pathParts.length - 1] || '';
            
            // Extraire la clé du paramètre ?key=
            const extractedKey: string | null = wpUrl.searchParams.get('key');

            if (extractedOrderId && extractedKey && /^\d+$/.test(extractedOrderId)) {
              // ✅ Redirection avec orderId ET orderKey
              router.push(`/checkout/order-received/${extractedOrderId}?key=${extractedKey}`);
              await finalizeCheckout(checkout);
              return checkout;
            }
          } catch (e) {
            console.error('Échec de l\'analyse de l\'URL de redirection', e);
          }
      ...
      if (checkout?.redirect && isPayPalPayment()) {
        ...
        // ✅ 3. Redirection standard : orderId dans le chemin + orderKey en query param
