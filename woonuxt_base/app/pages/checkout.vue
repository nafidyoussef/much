...
<template>
  <div class="flex flex-col min-h-screen bg-gray-50">
    <template v-if="cart && customer">
      <div v-if="cart.isEmpty" class="flex flex-col items-center justify-center flex-1 px-4">
        <Icon name="ion:cart-outline" size="120" class="mb-4 opacity-25 text-gray-500" />
        <h2 class="mb-2 text-2xl font-bold text-gray-800">{{ $t('shop.cartEmpty') }}</h2>
        <p class="mb-4 text-gray-500">{{ $t('shop.addProductsInYourCart') }}</p>
        <NuxtLink to="/products" class="px-6 py-3 font-semibold text-white rounded-lg bg-primary hover:bg-primary-dark transition-colors">
          {{ $t('shop.browseOurProducts') }}
        </NuxtLink>
      </div>

      <form v-else class="checkout-container container max-w-6xl mx-auto px-2 py-8 grid lg:grid-cols-3 gap-8" @submit.prevent="payNow">
        <div class="checkout-form grid w-full gap-6 lg:col-span-2">
          
          <div v-if="viewer" class="checkout-section">
            <h1 class="text-2xl font-semibold leading-none text-gray-900">Bon retour, {{ viewerSummary?.firstName || 'Client' }}</h1>
            <p v-if="viewerEmail" class="text-sm text-gray-600 mt-2">
              <span class="text-gray-400">Email: </span><span class="truncate">{{ viewerEmail }}</span>
            </p>
          </div>

          <div v-if="!viewer" class="checkout-section">
            <h1 class="text-2xl font-semibold leading-none text-gray-900">Paiement en tant qu'invité</h1>
            <div class="flex justify-between items-center gap-4 mt-2 cursor-pointer" @click="navigateToLogin(route.fullPath)">
              <p class="text-sm text-gray-600">Ou connectez-vous pour utiliser vos informations enregistrées.</p>
              <Button type="button" size="sm" variant="outline">Se connecter</Button>
            </div>
          </div>

          <!-- Billing Details -->
          <div class="checkout-section">
            <h3 class="text-xl font-semibold leading-none mb-4">{{ $t('billing.billingDetails') }}</h3>
            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('billing.email') }} <span class="text-red-500">*</span></label>
                <input v-model="formData.billing.email" type="email" required class="checkout-input" :class="{ 'has-error': isInvalidEmail }" @blur="checkEmailOnBlur" @input="checkEmailOnInput" />
                <p v-if="isInvalidEmail" class="mt-1 text-sm text-red-500">Email invalide</p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Téléphone <span class="text-red-500">*</span></label>
                <input v-model="formData.billing.phone" type="tel" placeholder="06 12 34 56 78" required class="checkout-input" :class="{ 'has-error': isInvalidPhone }" @blur="checkPhoneOnBlur" @input="checkPhoneOnInput" />
                <p v-if="isInvalidPhone" class="mt-1 text-sm text-red-500">Numéro invalide</p>
              </div>
              <div class="grid sm:grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('billing.firstName') }} <span class="text-red-500">*</span></label>
                  <input v-model="formData.billing.firstName" type="text" required class="checkout-input" />
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('billing.lastName') }} <span class="text-red-500">*</span></label>
                  <input v-model="formData.billing.lastName" type="text" required class="checkout-input" />
                </div>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('billing.address1') }} <span class="text-red-500">*</span></label>
                <input v-model="formData.billing.address1" type="text" placeholder="Ex: 123 Rue Hassan II" required class="checkout-input" />
              </div>
              
              <!-- ✅ City Dropdown Amélioré -->
              <div ref="cityInputRef" class="relative">
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('billing.city') }} <span class="text-red-500">*</span></label>
                <div class="relative">
                  <input 
                    v-model="citySearch" 
                    type="text" 
                  ...
                  <!-- Dropdown Liste des villes -->
                  <div 
                    v-if="showCityDropdown && filteredCities.length > 0" 
                    class="absolute z-30 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto custom-scrollbar"
                  >
                    <button 
                      v-for="city in filteredCities" 
                      :key="city" 
                      type="button" 
                      class="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-primary/10 hover:text-primary transition-colors flex items-center justify-between group"
                      @click="selectCity(city, false)"
                    >
                      <span>{{ city }}</span>
                      <Icon v-if="formData.billing.city === city" name="ion:checkmark" class="text-primary opacity-0 group-hover:opacity-100 transition-opacity" size="16" />
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="checkout-input flex items-center gap-2 bg-gray-50 text-gray-600 cursor-not-allowed select-none">
                  <span class="text-xl">🇲🇦</span> <span class="font-medium">Maroc</span>
                </div>
              </div>
            </div>
            
            <div v-if="shouldShowShippingFlow" class="flex items-center gap-3 mt-6 pt-4 border-t border-gray-200">
              <input id="ship-to-different-address" v-model="shipToDifferentAddress" type="checkbox" class="w-4 h-4 text-primary rounded focus:ring-primary border-gray-300" />
              <label for="ship-to-different-address" class="text-sm font-medium text-gray-700 cursor-pointer">{{ $t('billing.differentAddress') }}</label>
            </div>
          </div>

          <!-- Shipping Details -->
          <div v-if="shipToDifferentAddress" class="checkout-section">
            <h3 class="text-xl font-semibold leading-none mb-4">{{ $t('general.shippingAddress') }}</h3>
            <div class="space-y-4">
              <div class="grid sm:grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('billing.firstName') }} <span class="text-red-500">*</span></label>
                  <input v-model="formData.shipping.firstName" type="text" required class="checkout-input" />
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('billing.lastName') }} <span class="text-red-500">*</span></label>
                  <input v-model="formData.shipping.lastName" type="text" required class="checkout-input" />
                </div>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('billing.address1') }} <span class="text-red-500">*</span></label>
                <input v-model="formData.shipping.address1" type="text" required class="checkout-input" />
              </div>
              
              <!-- ✅ Shipping City Dropdown Amélioré -->
              <div ref="shippingCityInputRef" class="relative">
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('billing.city') }} <span class="text-red-500">*</span></label>
                <div class="relative">
                  <input 
                    v-model="shippingCitySearch" 
                    type="text" 
                    placeholder="Rechercher votre ville..." 
                    class="checkout-input pr-10" 
                    @focus="showShippingCityDropdown = true" 
                    @input="handleCityInput(true)" 
                  />
                  <Icon 
                    v-if="formData.shipping.city" 
                    name="ion:checkmark-circle" 
                    class="absolute right-3 top-1/2 -translate-y-1/2 text-green-500 pointer-events-none" 
                    size="20" 
                  />
                  ...
                  <div 
                    v-if="showShippingCityDropdown && filteredShippingCities.length > 0" 
                    class="absolute z-30 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto custom-scrollbar"
                  >
                    <button 
                      v-for="city in filteredShippingCities" 
                      :key="city" 
                      type="button" 
                      class="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-primary/10 hover:text-primary transition-colors flex items-center justify-between group"
                    ...
                    >
                      <span>{{ city }}</span>
                      <Icon v-if="formData.shipping.city === city" name="ion:checkmark" class="text-primary opacity-0 group-hover:opacity-100 transition-opacity" size="16" />
                    </button>
                  </div>
                </div>
              </div>
          ...
          <!-- Shipping Methods -->
          <div v-if="shouldShowShippingFlow && hasAvailableShippingMethods && (cart as any)?.chosenShippingMethods?.[0]" class="checkout-section">
            <h3 class="text-xl font-semibold leading-none mb-4">{{ $t('general.shippingSelect') }}</h3>
            <ShippingOptions :options="(cart as any)?.availableShippingMethods?.[0]?.rates ?? []" :active-option="(cart as any).chosenShippingMethods[0]" />
          </div>

          <!-- Payment Methods -->
          <div v-if="checkoutPaymentGateways?.nodes?.length" class="checkout-section">
            <h3 class="text-xl font-semibold leading-none mb-4">{{ $t('billing.paymentOptions') }}</h3>
            <PaymentOptions :model-value="orderInput.paymentMethod" class="mb-4" :payment-gateways="checkoutPaymentGateways" @update:model-value="handleGatewaySelect" />
            <PaymentGatewayComponent :gateway="orderInput.paymentMethod" />
          </div>

          <!-- Order Note -->
          <div class="checkout-section">
            <h3 class="text-xl font-semibold leading-none mb-4">{{ $t('shop.orderNote') }} <span class="text-sm font-normal text-gray-500">({{ $t('general.optional') }})</span></h3>
            <textarea v-model="orderInput.customerNote" rows="3" class="checkout-input" :placeholder="$t('shop.orderNotePlaceholder')"></textarea>
          </div>
        </div>

        <!-- Order Summary -->
        <div class="lg:col-span-1">
          <div class="checkout-section sticky top-6">
            <h2 class="text-xl font-bold text-gray-900 mb-4">Récapitulatif</h2>
            <OrderSummary />
            <p v-if="checkoutError" role="alert" class="text-sm text-red-600 bg-red-50 p-3 rounded-lg mt-4 border border-red-200 flex items-start gap-2">
              <Icon name="ion:alert-circle" class="mt-0.5 shrink-0" />
              {{ checkoutError }}
            </p>
            <Button :loading="isProcessingOrder" :disabled="isCheckoutDisabled" size="lg" type="submit" class="mt-6 w-full font-semibold shadow-lg shadow-primary/20">
              {{ buttonText }}
            </Button>
            <p class="text-xs text-gray-500 text-center mt-4 flex items-center justify-center gap-1">
              <Icon name="ion:lock-closed" size="14" /> Paiement sécurisé. En commandant, vous acceptez nos CGV.
            </p>
          </div>
        </div>
      </form>
